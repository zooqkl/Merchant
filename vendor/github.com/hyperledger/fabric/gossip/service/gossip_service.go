...
func (g *gossipServiceImpl) InitializeChannel(chainID string, endpoints []string, support Support) {
	...
	if g.deliveryService[chainID] != nil {
		...
		if leaderElection {
			...
			g.staticStartDeliverForChannel(chainID,support.Committer)
		} else {
...
func (g *gossipServiceImpl) staticStartDeliverForChannel(chainID string, committer blocksprovider.LedgerInfo) {
	g.deliveryService[chainID].StartDeliverForChannel(chainID, committer, func () {
		g.deliveryService[chainID].StopDeliverForChannel(chainID)
		g.staticStartDeliverForChannel(chainID,committer)
