...
func GetVdbProvider(provider ledger.PeerLedgerProvider) (privacyenabledstate.DBProvider, error){
	if t, ok := provider.(*Provider); ok {
		return t.vdbProvider, nil
	} else {
		errStr := "cannot use this function with other provider"
		logger.Error(errStr)
		return nil, errors.New(errStr)
	}
}

func GetHistorydbProvider(provider ledger.PeerLedgerProvider) (historydb.HistoryDBProvider, error){
	if t, ok := provider.(*Provider); ok {
		return t.historydbProvider, nil
	} else {
		errStr := "cannot use this function with other provider"
		logger.Error(errStr)
		return nil, errors.New(errStr)
