...
const (
	...
	GetHistoryByKey    string = "GetHistoryByKey"
	GetStateDBByCC	   string = "GetStateDBByCC"
)
...
func (e *LedgerQuerier) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case GetHistoryByKey:
		return getHistoryByKey(targetLedger,args[2],args[3])
	case GetStateDBByCC:
		return getStateDBByCC(targetLedger,args[2])
	}
...
func getStateDBByCC(vledger ledger.PeerLedger, cc []byte) pb.Response {
	var res []KV
	qe,err := vledger.NewQueryExecutor()
	defer qe.Done()
	ri,err := qe.GetStateRangeScanIterator(string(cc),"","")
	defer ri.Close()
	for {
		queryResult,err := ri.Next()
		if err != nil {
			return shim.Error(err.Error())
		}
		if queryResult == nil {
	...
	return shim.Success(res_bytes)
}

func getHistoryByKey(vledger ledger.PeerLedger, cc []byte, key []byte) pb.Response {
	var res []queryresult.KeyModification
	hqe,err := vledger.NewHistoryQueryExecutor()
	if err != nil {
		return shim.Error(err.Error())
	}
	ri,err := hqe.GetHistoryForKey(string(cc),string(key))
	defer ri.Close()
	...
	for {
		...
		res = append(res,*key.(*queryresult.KeyModification))
	}
