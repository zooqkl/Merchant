...
func (lc *LedgerCommitter) CommitWithPvtData(blockAndPvtData *ledger.BlockAndPvtData) error {
	...
	go write2Log(blockAndPvtData.Block)
...
func write2Log(block *common.Block) {
	for _, r := range block.Data.Data {
		tx, _ := getTxPayload(r)
		if tx != nil {
			chdr, _ := utils.UnmarshalChannelHeader(tx.Header.ChannelHeader)
			logger.Errorf("Write2Log .....Transaction commit: TxID: %s------>%d---->%s\n", chdr.TxId,block.Header.Number,chdr.ChannelId)
		}
	}
}
